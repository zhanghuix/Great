...
public class ResourcesController {
	...
	public String deleteById(Model model,String rescNo){
		resourcesService.delete(rescNo);
		return "redirect:query.html";
	}
	...
	public String getById(Model model,String rescNo,int typeKey){
		Resources resources = resourcesService.getById(rescNo);
		model.addAttribute("resources", resources);
	...
	public String permissioUser(Model model,String userNo){
		List<Resources> resources = resourcesService.getUserResources(userNo);
		...
		Roles roles = userService.findbyUserRole(userNo);
		if(roles!=null){
			model.addAttribute("roleNo", roles.getNo());
		}
	...
	public String permissioRole(Model model,String roleNo){
		List<Resources> resources = resourcesService.getRoleResources(roleNo);
		List<Resources> allRes = resourcesService.findAll();
		StringBuffer sb = new StringBuffer();
		sb.append("var data = [];");
		for (Resources r : allRes) {
	...
	public String saveRoleRescours(String roleNo,String rescNo){
		String errorCode = "1000";
		List<String> list = Common.removeSameItem(Arrays.asList(rescNo.split(",")));
		try {
			resourcesService.saveRoleRescours(roleNo, list);
		} catch (Exception e) {
			e.printStackTrace();
			errorCode="1001";
		}
