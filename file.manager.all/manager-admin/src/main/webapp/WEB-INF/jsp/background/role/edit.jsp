...
<script type="text/javascript"
	src="${ctx }/js/validform/Validform_v5.3.2.js"></script>
<script>
	  $(function () {
		  $("form").Validform({
			  tiptype: 3
...
<body>
	...
	<form
		...
		method="post">
		<input type="hidden" name="no" value="${role.no}">
				...
				<td>
					<div align="left" class="STYLE1" style="padding-left: 10px;">
						<input style="height: 20px; width: 200px" name="name" type="text"
							value="${role.name}" datatype="*" nullmsg="*角色名称不能为空!" /> <span
							class="Validform_checktip">*角色名</span>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div align="left" class="STYLE1" style="padding-left: 10px;">
						<input style="height: 20px; width: 200px" name="roleKey"
							type="text" value="${role.roleKey}" datatype="*"
							nullmsg="*角色roleKey不能为空!" /> <span class="Validform_checktip">*角色roleKey</span>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div align="left" class="STYLE1" style="padding-left: 10px;">
						<input name="enable" type="radio"
							<c:if test="${role.enable eq '0'}">checked="checked"</c:if>
							value="0" />:禁用 <input name="enable" type="radio"
							<c:if test="${role.enable eq '1'}">checked="checked"</c:if>
							value="1" />:正常
					</div>
				</td>
			</tr>
